...
\centering
\begin{align*}
&\substpath{Path}{String}{Path}:\mIt{Path} = \\
&\quad??? \\
&\substconstr{Constraint}{String}{Path}\mathit{: Constraint} = \\
&\quad??? \\
&\substconstrs{List[Constraint]}{String}{Path}\mathit{: List[Constraint]} = \\
&\quad??? \\
...
&\quad\mathit{cs2.foreach(entails(cs1, \_))}
\end{align*}
