...
Some of these problems can be easily described as a satisfiability ptroblem
...
These logics support more epressive language features including
...
allow the development of procedures that exploit propoerties of the fragment for efficiency.
A SMT solver is any software implementing a procedure for determining satisfiability modulo a given theory.
SMT solvers can be distinguished based on the underlying logic (first-order, modal, temportal, ...),
...
\begin{itemize}
    ...
    \item develping a common in- and output language for SMT solvers
...
Classes tend to be a too isolated unit to achieve modulatiry.
Desired functionality involes groups of related classes.
Grouping mechanisms exist, like namespaces in C++ and packages in Java,
but these mechanisms do not cover inhertiance and polymorphism for expressing variability.
...
requires the modifcation this class and its subclasses,
...
and through this outlining apossible implementation.
