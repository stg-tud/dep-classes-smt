...
we also define two extra sorts.
We define sort \mIt{CList} for \Constrs
and sort \mIt{CsList} for \Constrss.
...
\end{figure}\\
A SMTLib translation for substitution is given in \Cref{fig:smtlib-funs-subst}
and for generalization for \Cref{fig:smtlib-funs-gen}.
%
\begin{figure}[h]
\begin{lstlisting}[language=smtlib]
...
(declare-fun path-exists (Path) Bool)
// DCC Properties
...
(define-fun-rec big-or-Entails ((ccs CsList) (cs CList)) Bool
  (ite (is-cons ccs)
    (or (Entails cs (hd ccs))
     ...
     false))
\end{lstlisting}
\caption{SMTLib Preprocess Helper Function}
\label{fig:smtlib-preproc-helper}
\end{figure}\quad\\
%
