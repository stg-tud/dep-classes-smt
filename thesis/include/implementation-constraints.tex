...
\end{lstlisting}
- trait SMTSolver
  - defines functionallity necessary for a SMT solver
  - responsible for process handling
  - in and output of the solver.
- class Z3Solver implements SMTSolver
- function entails
  - preprocess: lookup + subst
  - calls the solver
\begin{lstlisting}[caption={Entailment Function},label=lst:entails,captionpos=b,frame={lines}]
  ...
  def entails(
      ...
      val entailment = [...]
      val (variables, paths, classes) = [...]

      val solver: SMTSolver = new Z3Solver(Axioms.all)
      solver.addCommands(
        SMTLibConverter.makeAsserts(classes))
      solver.addCommands(
        SMTLibConverter.generateSubstRules(vars, pths))
      solver.addCommand(SMTLibConverter
          .makeProgramEntailmentLookupFunction(P, pths))
