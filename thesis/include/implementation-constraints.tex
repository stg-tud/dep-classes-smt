...
is done via calling the SMT solver as a subprocess and using
...
where variables occuring in $y$ are bound in $z$.
...
and \mIt{Entails} models the entailment judgement for multiple constraints.
...
%
\begin{example}[Program entailment lookup function structure]\quad\\ % TODO: s/structure/generation
\label{ex:lookup-fun}
In this example we sketch the generation process
of a lookup function that models the existence check in rule C-Prog.
The program for which we generate a lookup function contains the following
program entailment declarations.
\begin{align*}
&\progEnt{x}{\instOf{x}{B}}{\instOf{x}{A}}\\
&\progEnt{x}{\instOf{x}{C}, \instOf{x.f}{D}}{\instOf{x}{A}}\\
&\progEnt{x}{\instOf{x}{E}}{\instOf{x}{D}}
\end{align*}
Since program entailments are used to express inheritance
relations these form the following class hierarchy.
\begin{center}
\begin{tikzpicture}
  \node (A) at (-1,0) {A};
  \node (B) at (-3,-1.5) {B};
...
There can be more than one entailment with the same right-hand side constraint,
as seen in the given program entailments.
Since we do want to avoid shadowing of entailments,
...
With this we can give the following function definition:
\begin{align*}
...
\end{example}\quad\\
%
Since this solves only the non-algorithmic nature of the rule,
we still need to find a suitable substitution of $a$ to input into the lookup function.
We can incorporate the enumeration process into the generation
of the lookup function to resolve this.
% TODO
% - enumerate vars + paths in lookup function
% - give resulting C-Prog rule using lookup

C-Prog\\
- similiar problems as C-Subst\\
- non algorithmic: in-prog predicate\\
- hard to find substitution for the solver\\
- apply same technique for solution\\
- find way to express prog ent existence requirement in algorithmic way\\
- enumerate varnames and paths to get rid of the substitution problem\\
- look at how rule works\\
  - since prog ent are used for inheritance\\
  - to check entailment of some constraint it suffices to check for entailment of constraints of prog ent
  - seen as inheritance problem: to show that path p is instance of class C, it suffices to show that p is instance of subclass of C + field requirements of subclass

C-Perm\\
- non structural rule\\
- good for checking the property on existing entailments\\
- bad for generating permutations out of one entailment\\
- mostly used for closure with C-Ident in our model since list based\\
- introduce C-DirectIdent that omits weakening\\
- C-DirectIdent introduces new quantified var\\
  - not a problem because it can be directly matched against the rhs of the implication
  - checking the property elem of list is easy for the solver

\subsection{Scala Integration}
- for scala integration\\
  - translate model to SMTLib\\
  - implement prepocessing procedure for generating subst rules and prog ent lookup function\\
  - entailments can then be checked by calling SMT solver
