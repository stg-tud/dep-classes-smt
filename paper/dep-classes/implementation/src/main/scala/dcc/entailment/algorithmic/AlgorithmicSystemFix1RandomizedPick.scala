...
class AlgorithmicSystemFix1RandomizedPick(program: Program, debug: Int = 0) extends Entailment {
  override def typ: EntailmentSort = EntailmentSort.AlgorithmicFix1RandomizedPick
        ...
        if (!visitedSubst3.contains(conclusion) && conclusion.isInstanceOf[PathEquivalence]) {
          ...
          for (p <- Random.shuffle(context.flatMap(_.containedPaths))) {
            if (search(context, PathEquivalence(p1, p), visitedClass, visitedProg, visitedSubst1, visitedSubst2, conclusion::visitedSubst3, visitedSubst4) &&
              search(context, PathEquivalence(p2, p), visitedClass, visitedProg, visitedSubst1, visitedSubst2, conclusion::visitedSubst3, visitedSubst4)
            ) {
              return true
            }
          }
